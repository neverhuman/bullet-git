...
| `bullet-git-workspace` | `SafeGit` hardened command builder, `PrivateClone` lifecycle (§20.2), `ScopeGrant`, `RealRepository` capability API over real Git |
...
clock, 32-byte nonce hex) is recorded in the runtime dir, never inside the
repository tree.
...
| `apply_change` | `patches`: `[{path, contents_hex}]` (full-file contents, hex) | `applied`: count |
...
← {"id":2,"ok":{"applied":1}}
→ {"id":3,"method":"prepare_candidate","token":{...},"params":{"change_seed":"feat","mission":"demo"}}
← {"id":3,"ok":{"id":"can_…","base_commit":"d6d3…","head_commit":"9f2c…","tree_hash":"41ab…","patch_hash":"…", ...}}
```

Error codes: `UNAUTHORIZED`, `STALE_AUTHORITY`, `OUT_OF_SCOPE`,
`SYMLINK_FORBIDDEN`, `WORKTREE_FORBIDDEN`, `WRONG_REPOSITORY`,
`WRONG_BRANCH`, `SEQUENCER_ACTIVE`, `UNCLASSIFIED_UNTRACKED`,
`BASE_MISSING`, `CLEANUP_NONCE_MISMATCH`, `CLEANUP_RECEIPT_REQUIRED`,
